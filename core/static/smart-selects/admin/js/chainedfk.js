var chainedfk = {
    ...
    fill_field: function (val, init_value, elem_id, url, empty_label, auto_choose) {
        var $selectField = jQuery(elem_id),
            options = [];
        url = url + "/" + val + "/";

        var empty_option = jQuery('<option></option>')
            .attr('value', '')
            .text(empty_label);
        ...
        jQuery.getJSON(url, function (j) {
            ...
            jQuery.each(j, function (index, optionData) {
    ...
    init: function (chainfield, url, id, init_value, empty_label, auto_choose) {
        ...
        if (!jQuery(chainfield).hasClass("chained")) {
            val = jQuery(chainfield).val();
            fill_field(val, init_value, id, url, empty_label, auto_choose);
        }
        jQuery(chainfield).change(function () {
            ...
            if (localID.indexOf("__prefix__") > -1) {
                prefix = jQuery(this).attr("id").match(/\d+/)[0];
                localID = localID.replace("__prefix__", prefix);
            }

            start_value = jQuery(localID).val();
